...
def print_case(case_index, c, total_cases):
  print(f"({case_index+1} / {total_cases})\tDiffuse A: {
    ...
    str(c.diffuse_b).ljust(6)}\tP(Reaction): {
    str(c.p_reaction).ljust(6)}\tFeed rate: {
...
def init_grid():
  ...
  grid[floor(GRID_WIDTH/2 - seed_size/2):floor(GRID_WIDTH/2 + seed_size/2), floor(GRID_HEIGHT/2 - seed_size/2):floor(GRID_HEIGHT/2 + seed_size/2)] = [0, 1]
  return grid
...
def render_hud_to(surf, case_index, total_cases, case, step, fps):
  ...
  hud_pos[1] += 18
  HUD_FONT.render_to(surf, hud_pos, f"P(Reaction): {case.p_reaction}", text_color)
  hud_pos[1] += 18
...
while running:
  ...
  if not args.casenum and step >= 800:
